...
define("select-kit/components/email-group-user-chooser-header", ["exports", "select-kit/components/multi-select/multi-select-header", "@ember/object", "@ember/object/computed", "discourse-common/config/environment", "select-kit/templates/components/email-group-user-chooser-header"], function (_exports, _multiSelectHeader, _object, _computed, _environment, _emailGroupUserChooserHeader) {
  "use strict";
  ...
  var _default = _multiSelectHeader.default.extend({
    layout: _emailGroupUserChooserHeader.default,
    classNames: ["email-group-user-chooser-header"],
    hasHiddenItems: (0, _computed.gt)("hiddenItemsCount", 0),
    shownItems: (0, _object.computed)("hiddenItemsCount", function () {
      if (this.selectKit.noneItem === this.selectedContent || this.hiddenItemsCount === 0) {
        return this.selectedContent;
      }

      return this.selectedContent.slice(0, this.selectedContent.length - this.hiddenItemsCount);
    }),
    hiddenItemsCount: (0, _object.computed)("selectedContent.[]", "selectKit.options.autoWrap", "selectKit.isExpanded", function () {
      if (!this.selectKit.options.autoWrap || this.selectKit.isExpanded || this.selectedContent === this.selectKit.noneItem || this.selectedContent.length <= 1 || (0, _environment.isTesting)()) {
        return 0;
      } else {
        var selectKitHeaderWidth = this.element.offsetWidth;
        var choices = this.element.querySelectorAll(".selected-name.choice");
        var input = this.element.querySelector(".filter-input");
        var alreadyHidden = this.element.querySelector(".x-more-item");

        if (alreadyHidden) {
          var hiddenCount = parseInt(alreadyHidden.getAttribute("data-hidden-count"), 10);
          return hiddenCount + (this.selectedContent.length - (choices.length + hiddenCount));
        }
        ...
        var total = choices[0].offsetWidth + input.offsetWidth;
        var shownItemsCount = 1;
        var shouldHide = false;

        for (var i = 1; i < choices.length - 1; i++) {
          var currentWidth = choices[i].offsetWidth;
          var nextWidth = choices[i + 1].offsetWidth;
          var ratio = (total + currentWidth + nextWidth) / selectKitHeaderWidth;
...
define("select-kit/components/future-date-input-selector", ["exports", "select-kit/components/combo-box", "select-kit/components/future-date-input-selector/mixin", "I18n", "@ember/object", "@ember/object/computed", "@ember/utils"], function (_exports, _comboBox, _mixin, _I18n, _object, _computed, _utils) {
  "use strict";
          ...
          modifiers: [{
            ...
            fn: function fn(data) {
              ...
              if (wrapper) {
                ...
                var topPlacement = popperElement && popperElement.getAttribute("data-popper-placement").startsWith("top-");
                ...
                if (placementStrategy === "fixed") {
                  var rects = _this9.element.getClientRects()[0];

                  if (rects) {
                    var bodyRects = body && body.getClientRects()[0];
                    wrapper.style.position = "fixed";
                    wrapper.style.left = "".concat(rects.left, "px");

                    if (topPlacement && bodyRects) {
                      wrapper.style.top = "".concat(rects.top - bodyRects.height, "px");
                    } else {
                      wrapper.style.top = "".concat(rects.top, "px");
                    }
  ...
  var _default = _component.default.extend(_utils.default, (_dec = (0, _decorators.default)("selectKit.options.filterPlaceholder", "selectKit.options.translatedFilterPlaceholder"), (_obj = {
    ...
    actions: {
      ...
      onKeydown: function onKeydown(event) {
        ...
        if (event.keyCode === 13 && (!this.selectKit.highlighted || this.selectKit.enterDisabled)) {
          this.element.querySelector("input").focus();
...
define("select-kit/components/select-kit/select-kit-header", ["exports", "@ember/component", "select-kit/mixins/utils", "@ember/object", "discourse-common/lib/helpers", "@ember/runloop"], function (_exports, _component, _utils, _object, _helpers, _runloop) {
  ...
  var _default = _component.default.extend(_utils.default, {
    ...
    attributeBindings: ["tabindex", "ariaOwns:aria-owns", "ariaHasPopup:aria-haspopup", "ariaIsExpanded:aria-expanded", "headerRole:role", "selectedValue:data-value", "selectedNames:data-name", "buttonTitle:title"],
