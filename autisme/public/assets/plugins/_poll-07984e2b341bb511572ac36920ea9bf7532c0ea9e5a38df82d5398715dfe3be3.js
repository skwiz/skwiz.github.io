...
define("discourse/plugins/poll/widgets/discourse-poll", ["exports", "I18n", "discourse/plugins/poll/controllers/poll-ui-builder", "discourse/widgets/raw-html", "discourse/lib/ajax", "discourse/widgets/post", "discourse/widgets/widget", "discourse/plugins/poll/lib/even-round", "discourse/plugins/poll/lib/chart-colors", "virtual-dom", "discourse-common/lib/icon-library", "discourse/lib/load-script", "discourse/lib/ajax-error", "discourse/lib/formatter", "discourse/lib/round", "discourse/lib/show-modal"], function (_exports, _I18n, _pollUiBuilder, _rawHtml, _ajax, _post, _widget, _evenRound, _chartColors, _virtualDom, _iconLibrary, _loadScript, _ajaxError, _formatter, _round, _showModal) {
  ...
  (0, _widget.createWidget)("discourse-poll-load-more", {
    ...
    click: function click() {
      ...
      state.loading = true;
      return this.sendWidgetAction("fetchVoters", attrs.optionId).finally(function () {
  ...
  (0, _widget.createWidget)("discourse-poll-voters", {
    ...
    html: function html(attrs) {
      var contents = attrs.voters.map(function (user) {
    ...
    html: function html(attrs) {
      ...
      if (options) {
        ...
        return ordered.map(function (option, idx) {
          ...
          if (isPublic) {
            contents.push(_this.attach("discourse-poll-voters", {
  ...
  (0, _widget.createWidget)("discourse-poll-number-results", {
    ...
    html: function html(attrs) {
      var poll = attrs.poll;
      var totalScore = poll.options.reduce(function (total, o) {
  ...
  (0, _widget.createWidget)("discourse-poll-container", {
    ...
    buildKey: function buildKey(attrs) {
      return "poll-container-".concat(attrs.id);
    },
    defaultState: function defaultState() {
      return {
        voters: []
    ...
    html: function html(attrs, state) {
      ...
      if (attrs.showResults) {
        ...
        var type = poll.type === "number" ? "number" : "standard";
        ...
        _contents.push((0, _virtualDom.h)("ul", options.map(function (option) {
          return _this2.attach("discourse-poll-option", {
            option: option,
    ...
    fetchVoters: function fetchVoters(optionId) {
      var _this3 = this;
      ...
      return (0, _ajax.ajax)("/polls/voters.json", {
        ...
        var voters = optionId ? state.voters[optionId] : state.voters;
        var newVoters = optionId ? result.voters[optionId] : result.voters;
        var votersSet = new Set(voters.map(function (voter) {
        ...
        if (attrs.poll.type === "regular") {
          Object.keys(state.voters).forEach(function (otherOptionId) {
            if (optionId !== otherOptionId) {
              state.voters[otherOptionId] = state.voters[otherOptionId].filter(function (voter) {
                return !votersSet.has(voter.username);
              });
            }
          });
        }

        _this3.scheduleRerender();
      }).catch(function (error) {
        if (error) {
          (0, _ajaxError.popupAjaxError)(error);
        } else {
    ...
    html: function html(attrs) {
      ...
      if (attrs.isMultiple) {
        if (attrs.showResults || attrs.isClosed) {
          var totalVotes = poll.options.reduce(function (total, o) {
          ...
          var help = this.multipleHelpText(attrs.min, attrs.max, poll.options.length);
    ...
    init: function init(attrs) {
      ...
      (0, _loadScript.default)("/javascripts/Chart.min.js").then(function () {
        ...
        var config = pieChartConfig(data, labels, {
          legendContainerId: "poll-results-legend-".concat(attrs.id)
        });
        var el = document.getElementById("poll-results-chart-".concat(attrs.id)); // eslint-disable-next-line no-undef

        _this4._chart = new Chart(el.getContext("2d"), config);
      });
    },
    willRerenderWidget: function willRerenderWidget() {
      var _this$_chart;

      (_this$_chart = this._chart) === null || _this$_chart === void 0 ? void 0 : _this$_chart.destroy();
    },
    buildAttributes: function buildAttributes(attrs) {
      return {
  ...
  (0, _widget.createWidget)("discourse-poll-pie-chart", {
    ...
    html: function html(attrs) {
      ...
      contents.push(chart);
      contents.push((0, _virtualDom.h)("ul#poll-results-legend-".concat(attrs.id, ".pie-chart-legends")));
      return contents;
    }
  });
  var htmlLegendPlugin = {
    ...
    afterUpdate: function afterUpdate(chart, args, options) {
      var ul = document.getElementById(options.containerID);
      ul.innerHTML = "";
    ...
    return {
      ...
      options: {
        ...
        plugins: {
          legend: {
            labels: {
              generateLabels: function generateLabels() {
                return labels.map(function (text, index) {
                  return {
          ...
          htmlLegend: {
            containerID: opts === null || opts === void 0 ? void 0 : opts.legendContainerId
          }
        }
      }
    };
  ...
  (0, _widget.createWidget)("discourse-poll-buttons", {
    ...
    html: function html(attrs) {
      ...
      if (poll.close) {
        var closeDate = moment(poll.close);
  ...
  var _default = (0, _widget.createWidget)("discourse-poll", {
    ...
    defaultState: function defaultState(attrs) {
      ...
      var showResults = poll.results !== "on_close" && this.hasVoted() && !staffOnly;
      return {
        loading: false,
    ...
    min: function min() {
      var min = parseInt(this.attrs.poll.min, 10);
    ...
    max: function max() {
      var max = parseInt(this.attrs.poll.max, 10);
      var numOptions = this.attrs.poll.options.length;
    ...
    toggleStatus: function toggleStatus() {
      ...
      bootbox.confirm(_I18n.default.t(this.isClosed() ? "poll.open.confirm" : "poll.close.confirm"), _I18n.default.t("no_value"), _I18n.default.t("yes_value"), function (confirmed) {
        if (confirmed) {
          state.loading = true;
          var status = _this5.isClosed() ? "open" : "closed";
    ...
    toggleOption: function toggleOption(option) {
      ...
      if (!this.isMultiple()) {
        return this.castVotes().catch(function () {
          return _this6._toggleOption(option);
        });
      }
    },
    castVotes: function castVotes() {
      ...
      return (0, _ajax.ajax)("/polls/vote", {
        ...
        _this7.appEvents.trigger("poll:voted", poll, attrs.post, attrs.vote);
        ...
        if (attrs.poll.results === "staff_only") {
          if (_this7.currentUser && _this7.currentUser.staff) {
            state.showResults = true;
          } else {
