  ...
  (0, _widget.createWidget)("discourse-poll-standard-results", {
    ...
    fetchVoters: function fetchVoters(optionId) {
      ...
      return _fetchVoters({
        ...
        var voters = state.voters[optionId];
        var newVoters = result.voters[optionId]; // remove users who changed their vote

        var newVotersSet = new Set(newVoters.map(function (voter) {
        ...
        Object.keys(state.voters).forEach(function (otherOptionId) {
          if (optionId !== otherOptionId) {
            state.voters[otherOptionId] = state.voters[otherOptionId].filter(function (voter) {
              return !newVotersSet.has(voter.username);
            });
          }
        });
    ...
    html: function html(attrs, state) {
      ...
      if (options) {
        ...
        return ordered.map(function (option, idx) {
          ...
          if (isPublic) {
            contents.push(_this2.attach("discourse-poll-voters", {
    ...
    html: function html(attrs) {
      ...
      if (attrs.showResults) {
        ...
        _contents.push((0, _virtualDom.h)("ul", options.map(function (option) {
          return _this4.attach("discourse-poll-option", {
            option: option,
    ...
    castVotes: function castVotes() {
      ...
      return (0, _ajax.ajax)("/polls/vote", {
        ...
        if (attrs.poll.results === "staff_only") {
          if (_this7.currentUser && _this7.currentUser.get("staff")) {
            state.showResults = true;
          } else {
