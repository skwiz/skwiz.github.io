...
Ember.TEMPLATES["javascripts/modal/discourse-local-dates-create-modal"] = Ember.HTMLBars.template({"id":null,"block":"{\"symbols\":[],\"statements\":[[1,[28,\"discourse-local-dates-create-form\",null,[[\"config\",\"insertDate\"],[[24,[\"config\"]],[24,[\"insertDate\"]]]]],false],[0,\"\\n\"]],\"hasEval\":false}","meta":{"moduleName":"javascripts/discourse/templates/modal/discourse-local-dates-create-modal"}});
define("discourse/plugins/discourse-local-dates/initializers/discourse-local-dates", ["exports", "discourse-common/lib/deprecated", "discourse-common/lib/get-owner", "discourse/lib/d-popover", "../lib/local-date-builder", "discourse/lib/plugin-api", "discourse/lib/show-modal", "discourse/lib/download-calendar", "discourse-common/lib/icon-library", "I18n"], function (_exports, _deprecated, _getOwner, _dPopover, _localDateBuilder, _pluginApi, _showModal, _downloadCalendar, _iconLibrary, _I18n) {
  "use strict";
  ...
  function applyLocalDates(dates, siteSettings) {
    ...
    var currentUserTZ = moment.tz.guess();
    dates.forEach(function (element) {
      var _element$classList;

      var opts = buildOptionsFromElement(element, siteSettings);
      var localDateBuilder = new _localDateBuilder.default(opts, currentUserTZ).build();
      element.innerText = "";
      element.insertAdjacentHTML("beforeend", "\n        <svg class=\"fa d-icon d-icon-globe-americas svg-icon\" xmlns=\"http://www.w3.org/2000/svg\">\n          <use xlink:href=\"#globe-americas\"></use>\n        </svg>\n        <span class=\"relative-time\">".concat(localDateBuilder.formated, "</span>\n      "));
      element.setAttribute("aria-label", localDateBuilder.textPreview);
      var classes = ["cooked-date"];
  ...
  function _rangeElements(element) {
    ...
    return Array.from(element.parentElement.children).filter(function (span) {
      return span.dataset.date;
    });
  }
  ...
  function initializeDiscourseLocalDates(api) {
    ...
    var defaultTitle = _I18n.default.t("discourse_local_dates.default_title", {
    ...
    if (chat) {
      ...
      api.modifyClass("component:chat-composer", {
        ...
        actions: {
          insertDiscourseLocalDate: function insertDiscourseLocalDate() {
            var insertDate = this.addText.bind(this);
    ...
    api.decorateCookedElement(function (elem, helper) {
      var _helper$getModel, _helper$getModel$topi;

      var dates = elem.querySelectorAll(".discourse-local-date");
      applyLocalDates(dates, siteSettings);
      var topicTitle = helper === null || helper === void 0 ? void 0 : (_helper$getModel = helper.getModel()) === null || _helper$getModel === void 0 ? void 0 : (_helper$getModel$topi = _helper$getModel.topic) === null || _helper$getModel$topi === void 0 ? void 0 : _helper$getModel$topi.title;
    ...
    api.modifyClass("component:d-editor", {
      ...
      actions: {
        insertDiscourseLocalDate: function insertDiscourseLocalDate(toolbarEvent) {
          (0, _showModal.default)("discourse-local-dates-create-modal").setProperties({
            insertDate: function insertDate(markup) {
  ...
  function buildHtmlPreview(element, siteSettings) {
    ...
    var calendarNode = _downloadCalendarNode(element);
  ...
  function calculateStartAndEndDate(startDataset, endDataset) {
    ...
    startDate = moment.tz("".concat(startDataset.date, " ").concat(startDataset.time || "").trim(), startDataset.timezone);

    if (endDataset) {
      endDate = moment.tz("".concat(endDataset.date, " ").concat(endDataset.time || "").trim(), endDataset.timezone);
    }
  ...
  function _downloadCalendarNode(element) {
    var _rangeElements$map = _rangeElements(element).map(function (dateElement) {
      return dateElement.dataset;
    }),
        _rangeElements$map2 = _slicedToArray(_rangeElements$map, 2),
        startDataset = _rangeElements$map2[0],
        endDataset = _rangeElements$map2[1];

    var _calculateStartAndEnd = calculateStartAndEndDate(startDataset, endDataset),
        _calculateStartAndEnd2 = _slicedToArray(_calculateStartAndEnd, 2),
        startDate = _calculateStartAndEnd2[0],
        endDate = _calculateStartAndEnd2[1];
    ...
    node.innerHTML = "".concat((0, _iconLibrary.renderIcon)("string", "file"), " ").concat(_I18n.default.t("download_calendar.add_to_calendar"));
  ...
  function _calculateDuration(element) {
    var _rangeElements$map3 = _rangeElements(element).map(function (dateElement) {
      return dateElement.dataset;
    }),
        _rangeElements$map4 = _slicedToArray(_rangeElements$map3, 2),
        startDataset = _rangeElements$map4[0],
        endDataset = _rangeElements$map4[1];

    var startDateTime = moment("".concat(startDataset.date, " ").concat(startDataset.time || "").trim());
    var endDateTime = moment("".concat(endDataset.date, " ").concat(endDataset.time || "").trim());
    var duration = endDateTime.diff(startDateTime, "minutes"); // negative duration is used when we calculate difference for end date from range
  ...
  var _default = {
    ...
    showDatePopover: function showDatePopover(event) {
      var _event$target, _event$target$classLi, _event$target2, _event$target2$classL;

      var owner = (0, _getOwner.getOwner)(this);

      if (owner.isDestroyed || owner.isDestroying) {
        return;
      }

      var siteSettings = owner.lookup("site-settings:main");

      if (event === null || event === void 0 ? void 0 : (_event$target = event.target) === null || _event$target === void 0 ? void 0 : (_event$target$classLi = _event$target.classList) === null || _event$target$classLi === void 0 ? void 0 : _event$target$classLi.contains("discourse-local-date")) {
      ...
      } else if (event === null || event === void 0 ? void 0 : (_event$target2 = event.target) === null || _event$target2 === void 0 ? void 0 : (_event$target2$classL = _event$target2.classList) === null || _event$target2$classL === void 0 ? void 0 : _event$target2$classL.contains("download-calendar")) {
    ...
    hideDatePopover: function hideDatePopover(event) {
      var _event$target3, _event$target3$classL;

      if (event === null || event === void 0 ? void 0 : (_event$target3 = event.target) === null || _event$target3 === void 0 ? void 0 : (_event$target3$classL = _event$target3.classList) === null || _event$target3$classL === void 0 ? void 0 : _event$target3$classL.contains("discourse-local-date")) {
        (0, _dPopover.hidePopover)(event);
      }
    },
  ...
  */
  var DateWithZoneHelper = /*#__PURE__*/function () {
    ...
    _createClass(DateWithZoneHelper, [{
      ...
      key: "unitRepetitionsBetweenDates",
      value: function unitRepetitionsBetweenDates(duration, date) {
        var _duration$split = duration.split("."),
            _duration$split2 = _slicedToArray(_duration$split, 2),
            count = _duration$split2[0],
            unit = _duration$split2[1]; // get the diff in the specified units with decimals


        var diff = Math.abs(this.datetime.diff(date, unit, true)); // get integer count of duration in diff, eg: 4 hours diff is 2 for 2.hours duration

        var integer = Math.trunc(diff / count); // get fractional to define if we have to add one "duration"

        var fractional = diff / count % 1;
...
define("discourse/plugins/discourse-local-dates/lib/local-date-builder", ["exports", "./date-with-zone-helper", "I18n", "discourse-common/lib/icon-library"], function (_exports, _dateWithZoneHelper, _I18n, _iconLibrary) {
  "use strict";
  ...
  var DATETIME_FORMAT = "LLL";
  var DATE_FORMAT = "LL";
  var FULL_DATETIME_FORMAT = "LLLL";
  var TIME_FORMAT = "h:mm A";
  var DAY_OF_THE_WEEK_FORMAT = "dddd";
  var RANGE_SEPARATOR = "→";
  var TIME_ICON = "clock";
  var SHORT_FORMAT_DAYS_PERIOD = 1;
    ...
    _createClass(LocalDateBuilder, [{
      ...
      value: function build() {
        ...
        if (this.recurring && moment().isAfter(localDate.datetime)) {
          var type = this.recurring.split(".")[1];
          var repetitionsForType = localDate.unitRepetitionsBetweenDates(this.recurring, moment.tz(this.localTimezone));
      ...
      value: function _generatePreviews(localDate, displayedTimezone) {
        ...
        previewedTimezones.push({
          ...
          current: true,
          formated: this._createDateTimeRange(_dateWithZoneHelper.default.fromDatetime(localDate.datetime, localDate.timezone, this.localTimezone), this.time, this.duration)
        });
        ...
        timezones.forEach(function (timezone) {
          ...
          previewedTimezones.push({
            ...
            formated: _this2._createDateTimeRange(_dateWithZoneHelper.default.fromDatetime(localDate.datetime, localDate.timezone, timezone), _this2.time, _this2.duration)
          });
        });
      ...
      value: function _createDateTimeRange(startRange, time, duration) {
        var _this$_calculateDates = this._calculateDatesForRange(startRange, time, duration),
            _this$_calculateDates2 = _slicedToArray(_this$_calculateDates, 2),
            startDate = _this$_calculateDates2[0],
            endDate = _this$_calculateDates2[1];

        var formatElements = [startDate.format("".concat(DAY_OF_THE_WEEK_FORMAT, ", ").concat(DATE_FORMAT)), this._optionalTimeIcon(startDate, endDate), startDate.format(TIME_FORMAT)];
      ...
      key: "_shortFormat",
      value: function _shortFormat(startDate, endDate) {
        return endDate.datetime.diff(startDate.datetime, "days") < SHORT_FORMAT_DAYS_PERIOD;
      }
    }, {
      key: "_optionalTimeIcon",
      value: function _optionalTimeIcon(startDate, endDate) {
        if (!endDate || this._shortFormat(startDate, endDate)) {
          return "<br />".concat((0, _iconLibrary.renderIcon)("string", TIME_ICON));
        }
      }
    }, {
      key: "_endDateFormat",
      value: function _endDateFormat(startDate, endDate) {
        return this._shortFormat(startDate, endDate) ? TIME_FORMAT : FULL_DATETIME_FORMAT;
      }
    }, {
      key: "_calculateDatesForRange",
      value: function _calculateDatesForRange(date, time, duration) {
        ...
        var dates = [date, duration ? date.add(duration, "minutes") : date.add(24, "hours")];
